...
if __name__ == "__main__":
    ...
    # reconstructing either content or style image (if default = True means reconstructing content), if recontructing style, set to False
    ...
    parser.add_argument("--height", type=int, help="width of content and style images (-1 keep original)", default=500) # fixed height 500 pixels

    parser.add_argument("--saving_freq", type=int, help="saving frequency for intermediate images (-1 means only final)", default=1) # 1 is save every intermedaite image, -1 is only saving final image
    parser.add_argument("--model", type=str, choices=['vgg16', 'vgg19'], default='vgg19') # vgg19 for model
    # if lbfgs takes up to much memory space, can either switch to Adam optimizer, make pixel 500 to 250, or switch VGG19 to VGG16
    parser.add_argument("--optimizer", type=str, choices=['lbfgs', 'adam'], default='lbfgs') # lbfgs for optimizer
