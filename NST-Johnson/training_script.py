...
def train(training_config):
    ...
    # prepare data loader
    train_loader = utils.get_training_data_loader(
        training_config)  # pytorch dataset (resize MSCOCO dataset into 256 x 256, concatenate into batches, image normalization)

    # prepare neural networks
    transformer_net = TransformerNet().train().to(device)
    perceptual_loss_net = PerceptualLossNet(requires_grad=False).to(device)  # VGG-16
    ...
    style_img_set_of_feature_maps = perceptual_loss_net(style_img)  # pass style iamge through perceptual net
    target_style_representation = [utils.gram_matrix(x) for x in
                                   style_img_set_of_feature_maps]  # find style representation (set of gram matrices)
    ...
    for epoch in range(training_config['num_of_epochs']):
        for batch_id, (content_batch, _) in enumerate(train_loader):  # train_loader --> takes one batch
            # step1: Feed content batch through transformer net
            content_batch = content_batch.to(device)
            stylized_batch = transformer_net(content_batch)  # pass batch through transformer net --> stylized
            ...
            target_content_representation = content_batch_set_of_feature_maps.relu2_2  # content representation is different every iteration, while style representation is fixed
...
if __name__ == "__main__":
    ...
    parser.add_argument("--style_weight", type=float, help="weight factor for style loss", default=4e5)  # experiment
    parser.add_argument("--tv_weight", type=float, help="weight factor for total variation loss", default=0)
    parser.add_argument("--num_of_epochs", type=int, help="number of training epochs ", default=2)  # experiment
