...
def stylize_static_image(inference_config):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # check if device only have cpu/gpu support

    # Prepare the model - load the weights and put the model into evaluation mode
    stylization_model = TransformerNet().to(device) # image transform net
    training_state = torch.load(os.path.join(inference_config["model_binaries_path"], inference_config["model_name"])) # load pytorch binary model to figure out state
    state_dict = training_state["state_dict"]
    stylization_model.load_state_dict(state_dict, strict=True) # load weights inside model
    stylization_model.eval() # eval model - eval affects layers such as Dropout and BatchNorm - import for inference

    if inference_config['verbose']:
        utils.print_model_metadata(training_state)

    with torch.no_grad(): # forbids pytorch to calculate gradience (memory overhead)
        if os.path.isdir(inference_config['content_input']):  # do a batch stylization (every image in the directory)
            ...
            content_img_path = os.path.join(inference_config['content_images_path'], inference_config['content_input']) # create image path
            # prepare image: add batch dimension, do normalization
            content_image = utils.prepare_img(content_img_path, inference_config['img_width'], device)
            stylized_img = stylization_model(content_image).to('cpu').numpy()[0] # call stylized model on content image, paste to cpu and convert to numpy (extract zero b/c added batch dimension)
            utils.save_and_maybe_display_image(inference_config, stylized_img, should_display=inference_config['should_not_display']) # saves stylized image
...
if __name__ == "__main__":
    ...
    # checking if there are only pytorch models in the binary directory
    assert utils.dir_contains_only_models(model_binaries_path), f'Model directory should contain only model binaries.'
    os.makedirs(output_images_path, exist_ok=True) # output directory where images will be dumped
    ...
    parser.add_argument("--img_width", type=int, help="Resize content image to this width", default=500) # can also determine quality of result
    ...
    stylize_static_image(inference_config) # all the parameters are wrapped into this function