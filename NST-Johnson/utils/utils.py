...
# create numpy image, normalized 0-1 range
...
def prepare_img(img_path, target_shape, device, batch_size=1, should_normalize=True, is_255_range=False):
    img = load_image(img_path, target_shape=target_shape) # loads image from specified path

    transform_list = [transforms.ToTensor()]
    if is_255_range:
        transform_list.append(transforms.Lambda(lambda x: x.mul(255)))
    if should_normalize:
        # normalize transform (use IMAGENET_MEAN_1 and IMAGENET_standard_deviation to normalize image)
    ...
    img = transform(img).to(device) # push image onto GPU
    img = img.repeat(batch_size, 1, 1, 1) # add batch dimension in front b/c models are always expecting it
...
def post_process_image(dump_img):
    ...
    dump_img = (np.clip(dump_img, 0., 1.) * 255).astype(np.uint8) # np.clip() ensure image pixel values are between 0 - 1, then scale to 0-255, type is np.uint8
    dump_img = np.moveaxis(dump_img, 0, 2) #  rearrange the dimensions of the image array, moves the first axis to the last position,(channels, height, width) to (height, width, channels)
